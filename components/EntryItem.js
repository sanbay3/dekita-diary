...
export default function EntryItem({ entry, onDelete }) {
  ...
  return (
    <li className="flex items-start gap-3 rounded-lg bg-white px-4 py-3 shadow-sm dark:bg-zinc-800">
      <span
        className={`mt-0.5 shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${category.badge}`}
      >
        {category.label}
      </span>
      <span className="flex-1 break-words text-sm text-zinc-800 dark:text-zinc-100">
        {entry.text}
      </span>
      <button
        type="button"
